...
def add_user_name(powerline, username):
    bg = 155
    fg = 22
    powerline.append(Segment(powerline, username, fg, bg))

def add_hostmachine(powerline, host):
    bg = 184
    fg = 0
    powerline.append(Segment(powerline, host, fg, bg))

if __name__ == '__main__':
    p = Powerline(mode='patched')
    cwd = os.getcwd()
    add_user_name(p, getpass.getuser())
    add_hostmachine(p, platform.node())
